...
class AutocompleteResultText {
    ...
    var highlight: Array<AutocompleteResultTextHighlightItem> = emptyArray()
    var geo: AutocompleteResultGeo? = null

    fun getInfo() : String {
        var itemsToShow = ""

        for (item: AutocompleteResultTextHighlightItem in highlight) {
            itemsToShow += item.getInfo()
            itemsToShow += ","
        }

...
public class CircularKeyBuffer<T> {
    ...
    public CircularKeyBuffer(int size) {
        if (size <= 0) {
            Log.e("AJT", "Cannot create a circular buffer smaller than 1");
            mSize = -1;
    ...
    public void add(@NonNull T item) {
        if (item.equals("") | item.equals(0)) {
            Log.w("AJT", "Cannot add an empty item");
            return;
        }
    ...
    public T readFromEnd(Integer index) {
        ...
        if (!atLeastFourDataItems && index > mMostRecentItemIndex) {
            Log.w("AJT", "Not possible to read the item, as the buffer doesn't have at least "
                + (mMostRecentItemIndex + 1) + " items");
            return null;
        }

        if (index > mMaximumItemIndex) {
            Log.w("AJT", "Not possible to read the item, as the index is greater than the"
